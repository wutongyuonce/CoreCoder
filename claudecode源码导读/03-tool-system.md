...
```typescript
// src/Tool.ts (简化)
// 工具定义的类型，描述一个可供大模型调用的工具的所有元数据、行为、安全与展示逻辑
type ToolDef<Input, Output> = {
  // 工具的唯一标识名称，模型通过该名称选择调用哪个工具
  name: string

  // 工具的自然语言描述，供模型理解工具的用途，影响模型决策
  description: string

  // 使用 Zod 定义的输入参数 schema，用于校验模型传入的参数以及自动生成 JSON Schema 供模型参考
  inputSchema: ZodSchema<Input>

  // 工具的核心执行逻辑，接收解析后的输入参数和执行上下文，返回一个异步生成器
  // 生成器可以逐步产生工具执行的进度信息（ToolProgress），最终产出工具结果（ToolResult）
  call(input: Input, ctx: ToolUseContext): AsyncGenerator<ToolProgress | ToolResult>

  // ----- 安全相关 -----
  // 标识该工具是否为只读操作（不修改外部状态），用于决定是否需要用户确认或限制调用
  isReadOnly(): boolean

  // 根据输入参数返回该次调用所需的权限列表，用于权限检查与用户授权
  getPermissions(input: Input): ToolPermission[]

  // 可选的输入校验钩子，可在执行前进行更精细的验证（不依赖权限系统）
  validateInput?(input: Input, ctx: ToolUseContext): ValidationResult

  // 检查当前上下文是否满足该工具所需的权限，返回权限校验结果
  checkPermissions(input: Input, ctx: ToolUseContext): PermissionResult

  // ----- UI 展示相关 -----
  // 可选的渲染函数，用于在 UI 中展示工具将要执行的操作（例如展示参数摘要）
  renderToolUse?(input: Input): ReactNode

  // 可选的渲染函数，用于在 UI 中展示工具执行的结果（例如格式化输出数据）
  renderToolResult?(result: Output): ReactNode

  // ----- 上下文经济（Context Economy）相关 -----
  // 可选的摘要函数，用于将工具的输入和结果压缩成简短字符串，用于保留在对话上下文中而非完整保留详细输出
  getToolUseSummary?(input: Input, result: Output): string

  // 工具返回结果允许的最大字符数，超过该长度可能需要截断或摘要处理，防止上下文溢出
