...
def _exec_tools_parallel(self, tool_calls, on_tool=None):
    ...
    该方法会先同步触发 on_tool 回调（若提供），然后使用线程池并发执行所有工具，
    最后按原始顺序返回每个工具的执行结果。

    Args:
        tool_calls: 可迭代对象，每个元素包含工具名称和参数（通常为 ToolCall 类型）
        on_tool: 可选回调函数，签名为 on_tool(tool_name, arguments)，在每个工具执行前被调用

    Returns:
        list: 每个工具调用的执行结果，顺序与 tool_calls 中的顺序一致

    Note:
        线程池的最大工作线程数固定为 8，适合 IO 密集型的工具操作。
        如果任何一个工具调用抛出异常，f.result() 会重新抛出该异常。
    """
    # 遍历所有待执行的工具调用，同步触发回调（通常用于日志记录或前置处理）
    for tc in tool_calls:
        if on_tool:
            on_tool(tc.name, tc.arguments)

    # 使用线程池并发执行 _exec_tool 方法
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
        # 为每个工具调用提交一个异步任务，保留 Future 对象列表
        futures = [pool.submit(self._exec_tool, tc) for tc in tool_calls]
        # 等待所有任务完成，并按原始顺序依次获取结果（阻塞）
