...
// 该类负责处理流式响应中的工具调用事件，将碎片化的工具输入 JSON 拼接完整后立即执行工具
class StreamingToolExecutor {
  // 存储尚未收到完整输入的未决工具调用
  // key: content_block 的索引，value: 部分完成的工具调用信息（id、name、累积的 JSON 字符串）
  private pendingBlocks = new Map<number, PartialToolUse>()
  // 存储已经开始执行（已提交权限检查与执行）的工具 Promise
  private runningTools: Promise<ToolResult>[] = []

  // 处理流式事件的主要方法，每收到一个事件就更新状态
  onStreamEvent(event: StreamEvent) {
    switch (event.type) {
      case 'content_block_start':
        // 当一个新内容块开始时，检查它是否为工具调用块
        if (event.content_block.type === 'tool_use') {
          // 为这个工具调用在 pendingBlocks 中创建占位记录，初始化空的 inputJson
          this.pendingBlocks.set(event.index, {
            id: event.content_block.id,
            name: event.content_block.name,
            inputJson: '', // 后续 delta 事件会逐步填充
          })
        }
        break

      case 'content_block_delta':
        // 收到内容增量，判断是否为工具输入的 JSON 片段
        if (event.delta.type === 'input_json_delta') {
          // 找到对应的未决工具调用记录
          const block = this.pendingBlocks.get(event.index)!
          // 将新收到的 JSON 片段追加到已有的 JSON 字符串中
          block.inputJson += event.delta.partial_json
        }
        break

      case 'content_block_stop': {
        // 当前内容块结束，意味着该工具调用的所有 JSON 输入都已传输完毕
        const block = this.pendingBlocks.get(event.index)
        if (block) {
          // 将累积的 JSON 字符串解析为对象
          const input = JSON.parse(block.inputJson)
          // **立即** 执行该工具（会经过权限检查），得到一个 Promise
          const promise = this.executeToolWithPermissionCheck(block, input)
          // 将该 Promise 存入 runningTools 数组，便于后续跟踪所有正在执行的工具
          this.runningTools.push(promise)
          // 从 pendingBlocks 中删除该工具调用记录，表示已提交执行
